        ...
        if (target instanceof Creep) {
            ...
            if (target instanceof StructureLab) {
                return (target.mineralType == this.data.resourceType || !target.mineralType) &&
                       target.store[this.data.resourceType] <= target.store.getCapacity - amount;
            }
            else if (target instanceof StructureNuker) {
                return this.data.resourceType == RESOURCE_GHODIUM &&
                       target.store[RESOURCE_GHODIUM] <= target.store.getCapacity - amount;
            }
            else if (target instanceof StructurePowerSpawn) {
                return this.data.resourceType == RESOURCE_POWER &&
                       target.store[RESOURCE_POWER] <= target.store.getCapacity - amount;
            }
        }
        return false;
    }
...
class TaskTransferAll extends Task {
    ...
    isValidTask() {
        for (let resourceType in this.creep.store) {
            ...
            let amountInCarry = this.creep.store[resourceType] || 0;
            if (amountInCarry > 0) {
                return true;
            }
        }
